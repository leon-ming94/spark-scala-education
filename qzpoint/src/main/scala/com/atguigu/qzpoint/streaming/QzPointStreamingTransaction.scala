...
/**
  * 知识点掌握度实时统计
  */
object QzPointStreamingTransaction {
    ...
    def main(args: Array[String]): Unit = {
        ...
        //TODO 设置kafka消费数据的参数  判断本地是否有偏移量  有则根据偏移量继续消费 无则重新消费
        ...
        //TODO 处理完业务逻辑后 手动提交offset维护到本地 mysql中
        //TODO 将业务逻辑与offset放到同一事务中
        //TODO 先将executor的数据聚合到driver端 使用collect 再提交offset
        //TODO 将业务逻辑处理 与 offset都放到driver端 一起处理
        //TODO 缺点 : 处理速度肯定会慢 / executor数据量大collect容易OOM
        stream.foreachRDD(rdd => {
            ...
            try {
                val dsStream: RDD[(String, String, String, String, String, String)] = rdd.filter(item => item.value().split("\t").length == 6)
                ...
                val groupRDD: RDD[(String, Iterable[(String, String, String, String, String, String)])] = dsStream.groupBy(item => item._1 + "-" + item._2 + "-" + item._3)
                //TODO 将executor端的数据聚合到driver端
                val groupTuples: Array[(String, Iterable[(String, String, String, String, String, String)])] = groupRDD.collect()
                //TODO 处理业务逻辑
                for (elem <- groupTuples) {
                    qzQuestionUpdate(elem._1, elem._2, sqlProxy, client) //对题库进行更新操作
                }
                //TODO 获取消费的offset
